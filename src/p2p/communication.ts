...
export type OnDataCallback = (socket: net.Socket, message: any, socketId: string) => void;
...
function startTcpServer(
    ...
    return new Promise((resolve, reject) => {
        const server = net.createServer((socket) => {
                ...
                while ((delimiterIndex = buffer.indexOf(P2P_MESSAGE_DELIMITER)) > -1) {
                    ...
                    if (jsonString) {
                        try {
                            ...
                            console.error(`[TCP Server] Error parseando JSON de ${socketId}:`, (e as Error).message);
                        }
                    }
                }
            });
            ...
            socket.on('error', (err) => console.error(`[TCP Server] Error en socket de ${socketId}:`, err.message));
...
type CreateTcpServerResult = Promise<{ server: net.Server; port: number } | false>;
...
export function sendMessage(socket: net.Socket, messageObject: object): boolean {
